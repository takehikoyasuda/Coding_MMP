...
-- so this file covers the two terminal branches (minimal model, Mori fibre
-- space) reachable without that generalization, plus the option/error gates.
...
-- the whole driver terminates as a Mori fibre space in exactly one recorded
...
assert(segreMMP#"terminationType" == "Mori fibre space");
...
print "OK multigraded driver: threefoldMMPData(S,1,H) reaches a Mori fibre space in one step, no flattening.";
...
assert(segreMMPExplicitB#"terminationType" == "Mori fibre space");
